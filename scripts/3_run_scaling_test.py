...
# 1. Load the scaling test specific configuration
CFG = yaml.safe_load(open('config/scaling_matrix.yaml'))
...
def build_id(ts, g,rr,pl,bs,mbt,dp,tp,q,rep):
    # ID for scaling test
    return f"{ts}__SCALE_G{g}__RR{rr}__PL{pl}__B{bs}"
...
for (g,rr,pl,bs,mbt,dp,tp,q) in matrix:
    ...
    for rep in range(repetitions):
        ...
        print(f"Running scaling experiment: {exp_id}")
        subprocess.run(['bash','scripts/run_single_scaling.sh', exp_id], env=env, check=True)
