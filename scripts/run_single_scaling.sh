...
trap 'echo "Cleaning up for experiment ${EXP_ID}..."; [ ! -z "$DMON_PID" ] && kill $DMON_PID || true; echo "Saving server logs..."; docker-compose logs > "$OUT_DIR/server.log" 2>&1; rm -f .env' EXIT
